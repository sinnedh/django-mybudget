...
class ExpenseForOrganisationManagerTests(BaseTests):
    ...
    def test_summed_by_category(self):
        ...
        self.assertEqual(categories[0], {'category_id': c1.id, 'amount_sum': Decimal('131.79')})
        self.assertEqual(categories[1], {'category_id': c2.id, 'amount_sum': Decimal('78.79')})

        categories = Expense.objects.summed_by_category(organisation=self.o2)
        self.assertEqual(len(categories), 1)
        self.assertEqual(categories[0], {'category_id': c3.id, 'amount_sum': Decimal('44.23')})
    ...
    def test_get_latest(self):
        ...
        expenses = Expense.objects.get_latest(organisation=self.o1, days=5)
        self.assertItemsEqual(expenses, [e1a, e1b])
        expenses = Expense.objects.get_latest(organisation=self.o1, days=30)
        self.assertItemsEqual(expenses, [e1a, e1b, e1c])

        expenses = Expense.objects.get_latest(organisation=self.o2, days=2)
    ...
    def test_get_latest_with_account(self):
        ...
        self.assertItemsEqual(expenses, [e1a])
        expenses = Expense.objects.get_latest(organisation=self.o1, account=self.a1, days=3)
        self.assertItemsEqual(expenses, [e1a])
        ...
        expenses = Expense.objects.get_latest(organisation=self.o1, account=self.a2, days=2)
        self.assertItemsEqual(expenses, [])
